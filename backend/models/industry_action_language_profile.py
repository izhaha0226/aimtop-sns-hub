...
class IndustryActionLanguageProfile(Base):
    __tablename__ = "industry_action_language_profiles"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    industry_category: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    platform: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    format_type: Mapped[str | None] = mapped_column(String(50), index=True)
    source_scope: Mapped[str] = mapped_column(String(50), default="industry_cache")
